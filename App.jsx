...
/* Remplacer entièrement le contenu d'une table par le tableau fourni (simple mais efficace pour un usage départemental) */
async function saveKey(table, value, accessToken) {
  ...
  try {
    ...
    if (value.length > 0) {
      ...
      await supaFetch(`/rest/v1/${table}`, { method: "POST", body: JSON.stringify(rows) }, accessToken);
    }
  } catch (e) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg-page)", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px 60px" }}>
        ...
        {tab === "controle" && <FicheControle artisans={artisans} controles={controles} setControles={setControles} agent={agent} accessToken={accessToken} />}
...
function Exploitants({ type, exploitants, setExploitants, controles, setControles, arrondissement, accessToken, agent }) {
  const t = useT();
  ...
  const emptyForm = type === "eaux"
    ? {
        ...
        numeroAutorisation: "", statut: "en_regle",
      ...
      <SectionHeader
        ...
        action={
          <div className="no-print" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button style={ghostBtn} onClick={() => setShowDeclaration((s) => !s)}>
              <ClipboardList size={14} /> {t("ficheDeclarationExploitantBtn")}
            </button>
            <button style={primaryBtn} onClick={() => setShowForm((s) => !s)}>
...
function DeclarationExploitant({ type, items, controles, setControles, arrondissement, accessToken, agent, onClose }) {
  ...
  const anneesDispo = [new Date().getFullYear() - 1, new Date().getFullYear(), new Date().getFullYear() + 1];
  ...
  return (
    <div className="no-print" style={{ ...cardStyle, marginBottom: 18 }}>
      ...
      {wizStep === "apercu" && savedRecord && (
        <div>
          <div style={{ display: "grid", gap: 8, fontSize: 12.5, background: "#fff", border: "1px solid var(--border-light)", borderRadius: 4, padding: 14, marginBottom: 14 }}>
            ...
            <ApercuRow label={type === "eaux" ? t("quantitePrelevee") : t("quantiteExtraite")} value={`${savedRecord.quantite} ${savedRecord.unite}`} />
            <ApercuRow label={t("taxeForfaitaireParUnite")} value={`${savedRecord.tauxForfaitaire.toLocaleString("fr-FR")} XAF / ${savedRecord.tauxUnite || savedRecord.unite}`} />
            <ApercuRow label={t("montantTaxeTotal")} value={<b>{savedRecord.taxeTotale.toLocaleString("fr-FR")} XAF</b>} bold />
          </div>

          <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", marginBottom: 6 }}>{t("repartition")}</div>
          <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 14, tableLayout: "fixed", fontSize: 12 }}>
            <tbody>
              <tr>
                <td style={th}>{t("beneficiaire")}</td>
                <td style={th}>{t("tauxApplicable")}</td>
          ...
          <div style={{ fontSize: 11, color: "var(--text-faint)", marginBottom: 14 }}>
            {t("ficheCreeePar")} <b>{savedRecord.agent}</b> {t("leMot")} {formatDateFR(savedRecord.date)}
          </div>
...
function FicheControleTechnique({ type, items, controles, setControles, arrondissement, accessToken, agent, onClose }) {
  ...
  return (
    <div className="no-print" style={{ ...cardStyle, marginBottom: 18 }}>
      ...
      {wizStep === "apercu" && savedRecord && (
        <div>
          <ControleTechniqueApercu record={savedRecord} />
...
function ControleEauApercu({ record: c, arrondissementLabel }) {
  ...
  return (
    <div className="print-area" style={{ ...cardStyle, marginBottom: 18, maxWidth: 800, width: "100%", margin: "0 auto 18px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ textAlign: "center", fontSize: 11, lineHeight: 1.6, marginBottom: 10 }}>
        <div style={{ fontWeight: 700 }}>RÉPUBLIQUE DU CAMEROUN</div>
        <div>Paix — Travail — Patrie</div>
        <div style={{ fontWeight: 700, marginTop: 4 }}>MINISTÈRE DES MINES, DE L'INDUSTRIE ET DU DÉVELOPPEMENT TECHNOLOGIQUE</div>
        <div style={{ fontWeight: 700 }}>DÉLÉGATION DÉPARTEMENTALE DE {(arrondissementLabel || "").toUpperCase() || "......................"}</div>
      </div>
      <img src={LOGO_CAMEROUN} alt="Armoiries du Cameroun" style={{ width: 40, height: "auto", display: "block", margin: "0 auto 10px" }} />
...
function FicheControleEau({ items, controles, setControles, arrondissement, accessToken, agent, onClose }) {
  ...
  return (
    <div className="no-print" style={{ ...cardStyle, marginBottom: 18 }}>
      ...
      {wizStep === "apercu" && savedRecord && (
        <div>
          <ControleEauApercu record={savedRecord} arrondissementLabel={current.label} />
          <div className="no-print" style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
            <button type="button" onClick={() => window.print()} style={primaryBtn}><Printer size={14} /> {t("telechargerPdf")}</button>
            <button type="button" onClick={startEdit} style={ghostBtn}><Pencil size={14} /> {t("modifier")}</button>
            <button type="button" onClick={() => removeFiche(savedRecord.id)} style={{ ...ghostBtn, color: "#A8542E", borderColor: "#E3B8A8" }}><Trash2 size={14} /> {t("supprimer")}</button>
