...
const FIELD_MAPS = {
  ...
  declarations: {
    ...
    fonctionEquipement: "fonction_equipement",
...
function toDb(table, obj) {
  ...
  for (const [k, v] of Object.entries(obj)) {
    if (k === "type") continue; // discriminateur local uniquement, pas stocké
    out[map[k] || k] = v;
  }
  return out;
}
function fromDb(table, row) {
  ...
  if (table === "declarations") out.type = "declaration";
  return out;
}
...
async function loadKey(table, fallback, accessToken) {
  ...
  try {
    const rows = await supaFetch(`/rest/v1/${table}?select=*&order=created_at.asc`, {}, accessToken);
...
const STATUT_JURIDIQUE_OPTIONS = ["ÉTABLISSEMENTS (ETS)", "S.A.R.L", "S.A", "S.A.S", "S.N.C"];
...
function Exploitants({ type, exploitants, setExploitants, arrondissement, accessToken, agent }) {
  ...
  const champsCommunsSupplementaires = {
    ...
    statutJuridique: STATUT_JURIDIQUE_OPTIONS[0], capitalSocial: "", nombreEmployes: "",
    nombreEquipements: "", typeEquipement: "", fonctionEquipement: "",
  ...
  const addExploitant = () => {
    ...
    if (!form.nomStructure.trim() || !form.responsableNom.trim() || !form.numeroAutorisation.trim() || !dateOk) {
      setError(t("champsObligatoiresStructure"));
      return;
    }
    const horodatage = new Date().toISOString();
      ...
      {showForm && (
        <div style={{ ...cardStyle, marginBottom: 18 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
            ...
            <div>
              <label style={labelStyle}>{t("fonctionEquipementLabel")}</label>
              <input style={inputStyle} value={form.fonctionEquipement} onChange={(e) => setForm({ ...form, fonctionEquipement: e.target.value })} />
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
        ...
        >
          <div style={{ background: "var(--bg-page)", width: "100%", maxWidth: 420, maxHeight: "88vh", overflowY: "auto", borderRadius: 6, padding: "24px 22px" }}>
            ...
            <div style={{ display: "grid", gap: 10, fontSize: 13 }}>
              ...
              <ProfileRow label={t("typeEquipementLabel")} value={profil.typeEquipement || "—"} />
              <ProfileRow label={t("fonctionEquipementLabel")} value={profil.fonctionEquipement || "—"} />
...
/* Section de saisie d'une checklist Oui/Non avec observation par ligne (formulaire) */
function ChecklistFormSection({ items, values, onChange, colYes = "Oui", colNo = "Non" }) {
  return (
    <div style={{ display: "grid", gap: 10 }}>
      {items.map((it, i) => (
...
function ChecklistPrintTable({ items, values, colYes = "Oui", colNo = "Non" }) {
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 14, tableLayout: "fixed" }}>
      <colgroup>
        <col style={{ width: "8%" }} /><col style={{ width: "42%" }} /><col style={{ width: "8%" }} /><col style={{ width: "8%" }} /><col style={{ width: "34%" }} />
      </colgroup>
      <tbody>
        <tr>
          <td style={th}>N°</td><td style={th}>Points de contrôle</td><td style={th}>{colYes}</td><td style={th}>{colNo}</td><td style={th}>Observations</td>
        </tr>
        {items.map((it, i) => {
...
function DeclarationExploitant({ type, items, controles, setControles, arrondissement, accessToken, agent, onClose }) {
  ...
  const submitDeclaration = () => {
    ...
    const record = {
      ...
      taxeTotale,
      devise: "XAF",
      nomExploitant: structureSel.nomExploitant || "",
      telephoneExploitant: structureSel.telephoneExploitant || "",
      niu: structureSel.niu || "",
      rccm: structureSel.rccm || "",
      siegeSocial: structureSel.siegeSocial || "",
      statutJuridique: structureSel.statutJuridique || "",
      capitalSocial: structureSel.capitalSocial || "",
      nombreEmployes: structureSel.nombreEmployes || "",
      nombreEquipements: structureSel.nombreEquipements || "",
      typeEquipement: structureSel.typeEquipement || "",
      fonctionEquipement: structureSel.fonctionEquipement || "",
      adresseCompleteType: structureSel.adresseCompleteType || "",
      adresseTelephone: structureSel.adresseTelephone || "",
      adresseEmail: structureSel.adresseEmail || "",
      adresseBoitePostale: structureSel.adresseBoitePostale || "",
      ...
      {wizStep === "form" && structureSel && (
        <div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
            ...
            <div>
              <label style={labelStyle}>{t("nomExploitantLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.nomExploitant || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("telephoneExploitantLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.telephoneExploitant || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("niuLabel")}</label>
            ...
            <div>
              <label style={labelStyle}>{t("rccmLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.rccm || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("siegeSocialLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.siegeSocial || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("statutJuridiqueLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.statutJuridique || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("capitalSocialLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.capitalSocial || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("nombreEmployesLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.nombreEmployes || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("nombreEquipementsLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.nombreEquipements || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("typeEquipementLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.typeEquipement || "—"} readOnly />
            </div>
            <div>
              <label style={labelStyle}>{t("fonctionEquipementLabel")}</label>
              <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.fonctionEquipement || "—"} readOnly />
            </div>
            {(structureSel.adresseCompleteType === "telephone" || structureSel.adresseCompleteType === "tous") && (
              <div>
                <label style={labelStyle}>{t("telephoneDoc")}</label>
                <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.adresseTelephone || "—"} readOnly />
              </div>
            )}
            {(structureSel.adresseCompleteType === "email" || structureSel.adresseCompleteType === "tous") && (
              <div>
                <label style={labelStyle}>{t("adresseMailLabel")}</label>
                <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.adresseEmail || "—"} readOnly />
              </div>
            )}
            {(structureSel.adresseCompleteType === "boitePostale" || structureSel.adresseCompleteType === "tous") && (
              <div>
                <label style={labelStyle}>{t("boitePostaleLabel")}</label>
                <input style={{ ...inputStyle, background: "var(--bg-subtle)", color: "var(--text-muted)" }} value={structureSel.adresseBoitePostale || "—"} readOnly />
              </div>
            )}
            <div>
          ...
          <div className="print-area" style={{ ...cardStyle, maxWidth: 800, width: "100%", margin: "0 auto 18px" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, marginBottom: 16 }}>
              <tbody>
                ...
                <ApercuRow label={t("arrondissementDoc")} value={savedRecord.arrondissement} />
                {type === "carrieres" && <ApercuRow label={t("materiauExtrait")} value={savedRecord.materiau} />}
                <ApercuRow label={t("nomExploitantLabel")} value={savedRecord.nomExploitant || "—"} />
                <ApercuRow label={t("telephoneExploitantLabel")} value={savedRecord.telephoneExploitant || "—"} />
                <ApercuRow label={t("niuLabel")} value={savedRecord.niu || "—"} />
                <ApercuRow label={t("rccmLabel")} value={savedRecord.rccm || "—"} />
                <ApercuRow label={t("siegeSocialLabel")} value={savedRecord.siegeSocial || "—"} />
                <ApercuRow label={t("statutJuridiqueLabel")} value={savedRecord.statutJuridique || "—"} />
                <ApercuRow label={t("capitalSocialLabel")} value={savedRecord.capitalSocial || "—"} />
                <ApercuRow label={t("nombreEmployesLabel")} value={savedRecord.nombreEmployes || "—"} />
                <ApercuRow label={t("nombreEquipementsLabel")} value={savedRecord.nombreEquipements || "—"} />
                <ApercuRow label={t("typeEquipementLabel")} value={savedRecord.typeEquipement || "—"} />
                <ApercuRow label={t("fonctionEquipementLabel")} value={savedRecord.fonctionEquipement || "—"} />
                {(savedRecord.adresseCompleteType === "telephone" || savedRecord.adresseCompleteType === "tous") && (
                  <ApercuRow label={t("telephoneDoc")} value={savedRecord.adresseTelephone || "—"} />
                )}
                {(savedRecord.adresseCompleteType === "email" || savedRecord.adresseCompleteType === "tous") && (
                  <ApercuRow label={t("adresseMailLabel")} value={savedRecord.adresseEmail || "—"} />
                )}
                {(savedRecord.adresseCompleteType === "boitePostale" || savedRecord.adresseCompleteType === "tous") && (
                  <ApercuRow label={t("boitePostaleLabel")} value={savedRecord.adresseBoitePostale || "—"} />
                )}
...
export default function App() {
  ...
  const handleLogin = async (agentProfile, accessToken) => {
    ...
    setView("dashboard");
    const [a, c, ca, pa, ex] = await Promise.all([
