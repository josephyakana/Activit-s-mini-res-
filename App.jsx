...
function Register({ onBack, onCreated }) {
  ...
  const login = slugifyLogin(form.nom, form.prenom, []);
  const score = passwordScore(password);
  ...
  const submit = async () => {
    ...
    try {
      ...
      const accessToken = authData.access_token;
      if (!accessToken) {
        setError("Compte créé. Vérifiez votre boîte mail pour confirmer l'adresse avant de vous connecter.");
        setLoading(false);
        return;
      }
      const newAgent = {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1C2B39", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 460, margin: "0 auto", padding: "30px 26px", borderRadius: 6 }}>
        ...
        <button type="button" onClick={submit} disabled={loading} style={{ ...primaryBtn, width: "100%", marginTop: 20, justifyContent: "center", opacity: loading ? 0.6 : 1 }}>
          {loading ? "Création en cours..." : "Valider la création du compte"}
        </button>
        <button
          type="button"
          onClick={onBack}
