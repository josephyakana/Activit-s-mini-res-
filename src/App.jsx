  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex" }}>
      ...
      >
        ...
        {/* Sélecteur de secteur : toujours visible dans le menu */}
        <div style={{ borderBottom: "1px solid #DCD1B8", padding: "14px 20px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, fontSize: 12, fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", color: "#5B5346" }}>
            <Building2 size={17} color="#C9962C" />
            <span>Sélecteur de secteur</span>
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            {[
              { id: "minier", label: "Section minière", icon: <Pickaxe size={16} /> },
              { id: "industriel", label: "Section industrielle", icon: <Building2 size={16} /> },
            ].map((item) => {
              const active = sector === item.id;
              return (
                <button key={item.id} type="button" onClick={() => onSelectSector(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 12px", borderRadius: 5, cursor: "pointer", textAlign: "left", border: active ? "1px solid #C9962C" : "1px solid #E3DAC6", background: active ? "#FFF9EA" : "#F8F4EA", color: "#1C2B39", fontFamily: "'IBM Plex Sans', sans-serif", fontSize: 13.5, fontWeight: active ? 600 : 500 }}>
                  <span style={{ color: active ? "#C9962C" : "#5B5346", display: "flex" }}>{item.icon}</span>
                  <span style={{ flex: 1 }}>{item.label}</span>
                  {active && <CheckCircle2 size={15} color="#C9962C" />}
                </button>
              );
            })}
          </div>
...
function ProfileEdit({ agent, agents, setAgents, setAgent, onClose }) {
  ...
  const submit = () => {
    ...
    const next = agents.map((a) => (a.id === agent.id ? updated : a));
    setAgents(next);
    setAgent(updated);
    saveKey(STORAGE_KEYS.agents, next);
    setSaved(true);
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 110, background: "#1C2B39", overflowY: "auto", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 460, margin: "0 auto", padding: "30px 26px", borderRadius: 6 }}>
        ...
        <div style={{ marginTop: 14, fontSize: 11.5, color: "#A99B7F" }}>
          Identifiant : <b style={{ color: "#5B5346" }}>{agent.login}</b> (le mot de passe ne se modifie pas ici)
        </div>
...
/* ---- Écran de connexion ---- */
function LoginScreen({ agents, setAgents, onLogin, onBack }) {
...
function SignIn({ agents, onLogin, onBack, onCreateAccount, justCreated }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    const foundByLogin = agents.find((a) => (a.login || "").toLowerCase() === login.trim().toLowerCase());
    if (!foundByLogin) {
      setError("Ce login ne correspond à aucun compte.");
    } else if (foundByLogin.password !== password) {
      setError("Le mot de passe ne coïncide pas avec ce login.");
    } else {
      onLogin(foundByLogin);
    }
  };
      ...
      >
        ...
        {justCreated && (
          <div style={{ background: "#EAF0E5", color: "#4A5D3A", fontSize: 12.5, padding: "8px 10px", borderRadius: 3, marginBottom: 16 }}>
            Compte créé avec succès. Connectez-vous avec votre identifiant et votre mot de passe.
          </div>
        )}

        <label style={{ display: "block", fontSize: 12.5, color: "#5B5346", marginBottom: 5 }}>Identifiant (login)</label>
        <input
          value={login}
          onChange={(e) => setLogin(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter") submit(e); }}
          style={inputStyle}
          placeholder="Ex : madamou"
        />

        <label style={{ display: "block", fontSize: 12.5, color: "#5B5346", margin: "16px 0 5px" }}>Mot de passe</label>
        <PasswordInput
          value={password}
        ...
        <button type="button" onClick={submit} style={{ ...primaryBtn, width: "100%", marginTop: 22, justifyContent: "center" }}>
          Se connecter
        </button>
        <button
          type="button"
...
function Register({ agents, setAgents, onBack, onCreated }) {
  const [form, setForm] = useState({
    nom: "",
    prenom: "",
    dateNaissance: "",
  ...
  const login = slugifyLogin(form.nom, form.prenom, agents.map((a) => (a.login || "").toLowerCase()));
  const score = passwordScore(password);
  ...
  const submit = () => {
    setError("");
    const requiredOk = form.nom.trim() && form.prenom.trim() && form.dateNaissance && !form.dateNaissance.startsWith("XXXX") && form.matricule.trim() && form.email.trim();
    if (!requiredOk) {
      setError("Veuillez renseigner tous les champs obligatoires (*).");
      return;
    }
    if (!strong) {
      setError("Le mot de passe doit atteindre le niveau «Fort» (barre verte) avant de continuer.");
      return;
    }
    if (password !== confirm) {
    ...
    <div style={{ minHeight: "100vh", background: "#1C2B39", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 460, margin: "0 auto", padding: "30px 26px", borderRadius: 6 }}>
        ...
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px dashed #DCD1B8" }}>
          <label style={labelStyle}>Identifiant (généré automatiquement)</label>
          <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={login} readOnly />

          <label style={{ ...labelStyle, marginTop: 14 }}>Mot de passe<RequiredMark /></label>
        ...
        {error && (
          <div style={{ color: "#A8542E", fontSize: 12.5, marginTop: 16 }}>{error}</div>
        )}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F1EBDD", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px 60px" }}>
        ...
        {tab === "operateurs" && <Operateurs artisans={artisans} setArtisans={setArtisans} arrondissement={arrondissement} accessToken={accessToken} />}
        ...
        {tab === "cartes" && <Cartes artisans={artisans} cartes={cartes} setCartes={setCartes} accessToken={accessToken} />}
...
function Operateurs({ artisans, setArtisans, arrondissement, accessToken }) {
      ...
      {showForm && (
        <div style={cardStyle}>
          <div style={{ display: "flex", gap: 16, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <div>
              <label style={labelStyle}>Photo de l'artisan</label>
            ...
            <div>
              ...
              <div style={{ position: "relative" }}>
                <input
                  ...
                  value={form.gpsSite}
                  onChange={(e) => setForm({ ...form, gpsSite: e.target.value })}
                />
                <button
                  type="button"
                  ...
                  onClick={() => {
                    ...
                    navigator.geolocation.getCurrentPosition(
                      (pos) => setForm((f) => ({ ...f, gpsSite: `${pos.coords.latitude.toFixed(5)}, ${pos.coords.longitude.toFixed(5)}` })),
...
function ArtisanProfile({ artisan, artisans, setArtisans, onClose, accessToken }) {
  ...
  const saveChanges = () => {
    const next = artisans.map((a) => (a.id === artisan.id ? { ...a, ...editForm } : a));
    setArtisans(next);
    saveKey(STORAGE_KEYS.artisans, next, accessToken);
    onClose();
  };
        ...
        <div style={{ padding: "22px 20px" }}>
          ...
          {mode !== "edit" ? (
            <div style={{ display: "grid", gap: 8, fontSize: 13.5, color: "#2A2622" }}>
              ...
              <ProfileRow label="Coordonnées GPS" value={artisan.gpsSite || "—"} />
            ...
            <div style={{ display: "grid", gap: 12 }}>
              ...
              <div>
                ...
                <input style={inputStyle} value={editForm.gpsSite} onChange={(e) => setEditForm({ ...editForm, gpsSite: e.target.value })} />
              </div>
              <div>
                <label style={labelStyle}>Type de substance à exploiter</label>
                <select
...
function DeclarationApercu({ record: c }) {
  ...
  return (
    <div style={{ ...cardStyle, marginBottom: 18, maxWidth: 800, width: "100%", margin: "0 auto 18px" }}>
      ...
      {showRepertoire && repResult && (
        <>
          ...
          <div className="no-print" style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 18 }}>
            <button type="button" onClick={() => window.print()} style={primaryBtn}><Printer size={14} /> Imprimer</button>
...
function CarteRectoVerso({ artisan, commune }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          {/* ============ RECTO ============ */}
          <div style={{ fontSize: 11, color: "#A99B7F", marginBottom: 6, fontFamily: "'IBM Plex Sans', sans-serif", textTransform: "uppercase", letterSpacing: "0.08em", textAlign: "center" }}>Recto</div>
          <div
            style={{
              width: 340,
              height: 214,
          ...
          {/* ============ VERSO ============ */}
          <div style={{ fontSize: 11, color: "#A99B7F", margin: "22px 0 6px", fontFamily: "'IBM Plex Sans', sans-serif", textTransform: "uppercase", letterSpacing: "0.08em", textAlign: "center" }}>Verso</div>
          <div
            style={{
              width: 340,
              height: 214,
...
function Cartes({ artisans, cartes, setCartes, accessToken }) {
  ...
  const sauvegarderCarte = () => {
    ...
    const next = savedArtisanIds.includes(genArtisanId) ? cartes : [...cartes, { id: uid(), artisanId: genArtisanId }];
    setCartes(next);
...
/* ---- Inspecteurs ---- */
function Agents({ agents, setAgents, currentAgent }) {
  const removeAgent = (id) => {
    if (!window.confirm("Supprimer ce compte inspecteur ?")) return;
...
--------------------------------------------------------------- */
function IndustrialDashboard({ agent, onOpenMenu }) {
  const modules = [
    ["Établissements classés", "Suivi des établissements, autorisations et contrôles réglementaires."],
    ["Unités industrielles", "Recensement et surveillance des installations industrielles."],
    ["Inspections", "Contrôles techniques, sécurité, environnement et constats."],
    ["Documents", "Fiches de contrôle, procès-verbaux et rapports de mission."],
  ];
  return (
    <div style={{ minHeight: "100vh", background: "#F1EBDD", color: "#2A2622", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <NationalStripe />
      <header style={{ background: "#1C2B39", color: "#fff", padding: "18px 20px" }}>
        <div style={{ maxWidth: 980, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
          <MenuButton onClick={onOpenMenu} dark />
          <div style={{ flex: 1 }}><div style={{ fontFamily: "'Fraunces', serif", fontSize: 20 }}>Section industrielle</div><div style={{ fontSize: 12.5, color: "#D8CFBE", marginTop: 3 }}>Délégation départementale du MINMIDT de la Bénoué</div></div>
          <Building2 size={24} color="#C9962C" />
        </div>
      </header>
      <main style={{ maxWidth: 980, margin: "0 auto", padding: "32px 20px 70px" }}>
        <div style={{ background: "#fff", border: "1px solid #DCD1B8", borderRadius: 8, padding: "22px 20px", marginBottom: 20 }}>
          <div style={{ fontSize: 11.5, textTransform: "uppercase", letterSpacing: "0.08em", color: "#C9962C", fontFamily: "'IBM Plex Mono', monospace" }}>Secteur sélectionné</div>
          <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: "clamp(26px, 5vw, 38px)", margin: "7px 0 8px", color: "#1C2B39" }}>Contrôle des activités industrielles</h1>
          <p style={{ margin: 0, color: "#5B5346", lineHeight: 1.6, fontSize: 14 }}>Espace dédié au suivi administratif et technique des établissements classés et des unités industrielles.</p>
          <div style={{ marginTop: 12, fontSize: 12.5, color: "#5B5346" }}>Inspecteur connecté : <b>{agent?.prenom ? `${agent.prenom} ${agent.nom}` : agent?.nom}</b></div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 14 }}>
          {modules.map(([title, text]) => <div key={title} style={{ background: "#fff", border: "1px solid #DCD1B8", borderRadius: 7, padding: 18, minHeight: 125 }}><div style={{ fontSize: 15, fontWeight: 700, color: "#1C2B39", marginBottom: 8 }}>{title}</div><div style={{ fontSize: 13, lineHeight: 1.55, color: "#5B5346" }}>{text}</div></div>)}
        </div>
      </main>
    </div>
  );
...
export default function App() {
  ...
  const [sector, setSector] = useState("minier");

  useEffect(() => {
    try {
      const saved = window.localStorage.getItem("bibemi-sector");
      if (saved === "minier" || saved === "industriel") setSector(saved);
    } catch {}
  }, []);

  const handleSelectSector = (nextSector) => {
    setSector(nextSector);
    try { window.localStorage.setItem("bibemi-sector", nextSector); } catch {}
    setMenuOpen(false);
  };
  ...
  return (
    <>
      ...
      {view === "dashboard" && agent && session && sector === "minier" && (
        <Dashboard
          agent={agent}
          accessToken={session.accessToken}
          artisans={artisans}
      ...
      {view === "dashboard" && agent && session && sector === "industriel" && (
        <IndustrialDashboard agent={agent} onOpenMenu={() => setMenuOpen(true)} />
      )}
    </>
  );
}
