...
/* ---------------------------------------------------------------
   Design tokens
   bg-cream #F1EBDD · ink #2A2622 · navy #1C2B39
  ...
  return (
    <button
      ...
      style={{
        ...
        height: 38,
        borderRadius: 6,
        border: dark ? "1px solid rgba(241,235,221,0.25)" : "1px solid #DCD1B8",
        background: dark ? "rgba(241,235,221,0.06)" : "#fff",
        color: dark ? "#F1EBDD" : "#1C2B39",
        display: "flex",
        alignItems: "center",
        justifyContent: "center",
        cursor: "pointer",
...
function MenuSection({ icon, label, badge, open, onToggle, children }) {
  return (
    <div style={{ borderBottom: "1px solid #DCD1B8" }}>
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {icon}
          <span style={{ fontSize: 14, color: "#1C2B39" }}>{label}</span>
          {!!badge && (
        ...
        <ChevronRight size={16} color="#A99B7F" style={{ transform: open ? "rotate(90deg)" : "rotate(0deg)", transition: "transform 0.15s", flexShrink: 0 }} />
      </button>
...
function MenuDrawer({
  ...
  const nonRegle = agent ? artisans.filter((a) => a.statut === "non_regle") : [];
  const nonConforme = agent ? controles.filter((c) => c.resultat === "non_conforme") : [];
  const cartesAExpirer = agent
    ? artisans
        .filter((a) => statutExpiration(a.dateExpiration))
        .sort((a, b) => joursAvantExpiration(a.dateExpiration) - joursAvantExpiration(b.dateExpiration))
    : [];
  const notifCount = nonRegle.length + nonConforme.length + cartesAExpirer.length;

  const mesFiches = agent ? controles.filter((c) => c.agent === agent.nom).slice(0, 5) : [];

  const q = search.trim().toLowerCase();
  const searchResults = q
    ? [
        ...artisans.filter((a) => a.nom.toLowerCase().includes(q)).slice(0, 4).map((a) => ({
          key: "a-" + a.id, label: a.nom, sub: a.site || "Artisan", tab: "operateurs",
        })),
        ...controles.filter((c) => c.operateur.toLowerCase().includes(q)).slice(0, 4).map((c) => ({
          key: "c-" + c.id, label: c.operateur, sub: "Fiche de déclaration · " + c.date, tab: "controle",
        })),
      ]
    : [];
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex" }}>
      ...
      >
        <NationalStripe />
        <div style={{ padding: "18px 20px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #DCD1B8" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <Seal size={34} />
            <div style={{ fontFamily: "'Fraunces', serif", fontSize: 16, color: "#1C2B39" }}>Menu</div>
          </div>
          <button type="button" onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#5B5346" }}>
            <X size={20} />
          </button>
        </div>

        {/* Recherche globale */}
        <div style={{ padding: "14px 20px", borderBottom: "1px solid #DCD1B8" }}>
          <div style={{ position: "relative" }}>
            <Search size={15} style={{ position: "absolute", left: 10, top: 10, color: "#A99B7F" }} />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Rechercher un artisan, une fiche..."
          ...
          {q && (
            <div style={{ marginTop: 8, display: "grid", gap: 4 }}>
              {searchResults.length === 0 && (
                <div style={{ fontSize: 12.5, color: "#A99B7F", padding: "6px 2px" }}>Aucun résultat.</div>
              )}
              {searchResults.map((r) => (
                <button
                  key={r.key}
                  type="button"
                  onClick={() => jump(r.tab)}
                  style={{ textAlign: "left", background: "#fff", border: "1px solid #E3DAC6", borderRadius: 4, padding: "7px 10px", cursor: "pointer" }}
                >
                  <div style={{ fontSize: 13, color: "#1C2B39", fontWeight: 600 }}>{r.label}</div>
                  <div style={{ fontSize: 11, color: "#5B5346" }}>{r.sub}</div>
                </button>
              ))}
            </div>
          )}
        ...
        <button type="button" onClick={onGoHome} style={menuRowStyle}>
          <HomeIcon size={18} color="#5B5346" />
          <span style={{ flex: 1, fontSize: 14, color: "#1C2B39" }}>Accueil</span>
          <ChevronRight size={16} color="#A99B7F" />
        </button>

        {agent && (
          <button type="button" onClick={onGoDashboard} style={menuRowStyle}>
            <LayoutDashboard size={18} color="#5B5346" />
            <span style={{ flex: 1, fontSize: 14, color: "#1C2B39" }}>Tableau de bord</span>
            <ChevronRight size={16} color="#A99B7F" />
          </button>
        )}
        ...
        {agent && (
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 12, padding: "16px 20px", background: "none", border: "none", borderBottom: "1px solid #DCD1B8", cursor: "pointer", textAlign: "left" }}
          >
            <div
              style={{
                width: 44, height: 44, borderRadius: "50%",
                background: agent.photo ? `url(${agent.photo}) center/cover` : "#DCD1B8",
                border: "1px solid #C9B98E", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
              }}
            >
              {!agent.photo && <Users size={19} color="#5B5346" />}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14.5, fontWeight: 600, color: "#1C2B39" }}>
                {agent.prenom ? `${agent.prenom} ${agent.nom}` : agent.nom}
              </div>
              <div style={{ fontSize: 12, color: "#5B5346" }}>Voir / modifier le profil</div>
            </div>
            <ChevronRight size={16} color="#A99B7F" />
          </button>
        )}
        ...
        {agent && (
          <MenuSection
            icon={<Bell size={18} color="#5B5346" />}
            label="Notifications"
            badge={notifCount}
            open={section === "notif"}
            onToggle={() => toggle("notif")}
          >
            {notifCount === 0 && <div style={{ fontSize: 12.5, color: "#A99B7F", padding: "6px 0" }}>Rien à signaler.</div>}
            <div style={{ display: "grid", gap: 6 }}>
              {cartesAExpirer.slice(0, 3).map((a) => {
                ...
                return (
                  <button key={"exp-" + a.id} type="button" onClick={() => jump("operateurs")} style={notifItemStyle}>
                    ...
                    <span style={{ fontSize: 12.5, color: "#1C2B39" }}>
                      {a.prenom ? `${a.prenom} ${a.nom}` : a.nom} — {expiree ? `carte expirée depuis ${Math.abs(jours)} j` : `carte à renouveler dans ${jours} j`}
                    </span>
                  </button>
                );
              })}
              {nonRegle.slice(0, 3).map((a) => (
                <button key={a.id} type="button" onClick={() => jump("operateurs")} style={notifItemStyle}>
                  ...
                  <span style={{ fontSize: 12.5, color: "#1C2B39" }}>{a.nom} — non en règle</span>
                </button>
              ))}
              {nonConforme.slice(0, 3).map((c) => (
                <button key={c.id} type="button" onClick={() => jump("controle")} style={notifItemStyle}>
                  <AlertTriangle size={14} color="#A8542E" />
                  <span style={{ fontSize: 12.5, color: "#1C2B39" }}>Fiche non conforme — {c.operateur}</span>
                </button>
              ))}
            </div>
          </MenuSection>
        ...
        {agent && (
          <MenuSection
            icon={<History size={18} color="#5B5346" />}
            label="Historique récent"
            open={section === "histo"}
            onToggle={() => toggle("histo")}
          >
            {mesFiches.length === 0 && <div style={{ fontSize: 12.5, color: "#A99B7F", padding: "6px 0" }}>Aucune fiche enregistrée par vous pour l'instant.</div>}
            <div style={{ display: "grid", gap: 6 }}>
              {mesFiches.map((c) => (
                <button key={c.id} type="button" onClick={() => jump("controle")} style={notifItemStyle}>
                  <ClipboardList size={14} color="#5B5346" />
                  <span style={{ fontSize: 12.5, color: "#1C2B39" }}>{c.date} · {c.operateur}</span>
                </button>
              ))}
            </div>
          </MenuSection>
        ...
        <MenuSection
          icon={<MapPin size={18} color="#5B5346" />}
          label={`Arrondissement · ${current?.label || ""}`}
          open={section === "arr"}
          onToggle={() => toggle("arr")}
        >
          <div style={{ display: "grid", gap: 6 }}>
            {ARRONDISSEMENTS.map((a) => (
              <button
                ...
                style={{
                  ...
                  color: "#1C2B39", fontSize: 14, cursor: "pointer", fontFamily: "'IBM Plex Sans', sans-serif",
                }}
              >
                <CommuneLogo id={a.id} size={26} />
                <span>
                  {a.label}
                  {a.id === arrondissement && <span style={{ color: "#C9962C", fontSize: 11.5, marginLeft: 6 }}>· actuel</span>}
                </span>
              </button>
            ))}
          </div>
        ...
        <MenuSection
          icon={<HelpCircle size={18} color="#5B5346" />}
          label="Aide / FAQ"
          open={section === "aide"}
          onToggle={() => toggle("aide")}
        >
          <div style={{ display: "grid", gap: 10, fontSize: 12.5, color: "#2A2622", lineHeight: 1.5 }}>
            <div><b>Ajouter un artisan :</b> onglet Artisans → « Ajouter un artisan ».</div>
            <div><b>Remplir une fiche :</b> onglet Fiche de déclaration, formulaire en bas de page.</div>
            <div><b>Générer une carte :</b> onglet Cartes → choisir l'artisan.</div>
            <div><b>Mot de passe oublié :</b> contactez un administrateur de la délégation.</div>
          </div>
        </MenuSection>

        <MenuSection
          icon={<Phone size={18} color="#5B5346" />}
          label="Contact / Assistance"
          open={section === "contact"}
          onToggle={() => toggle("contact")}
        >
          <div style={{ display: "grid", gap: 8, fontSize: 13, color: "#2A2622" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}><MapPin size={14} color="#C9962C" /> Délégation départementale, Bénoué</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Phone size={14} color="#C9962C" /> +237 6XX XXX XXX</div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Mail size={14} color="#C9962C" /> contact@delegation-benoue.cm</div>
          </div>
        </MenuSection>

        <MenuSection
          icon={<Info size={18} color="#5B5346" />}
          label="À propos"
          open={section === "apropos"}
          onToggle={() => toggle("apropos")}
        >
          <div style={{ fontSize: 12.5, color: "#2A2622", lineHeight: 1.6 }}>
            Ce site appuie la délégation départementale du MINMIDT dans le suivi et le contrôle
            des activités minières artisanales du département de la Bénoué : registre des
            artisans, fiches de déclaration de terrain et délivrance des cartes professionnelles,
            conformément à la réglementation minière en vigueur au Cameroun.
          </div>
        </MenuSection>

        <MenuSection
          icon={<Settings size={18} color="#5B5346" />}
          label="Paramètres"
          open={section === "parametres"}
          onToggle={() => toggle("parametres")}
        >
          <div style={{ display: "grid", gap: 10, fontSize: 13, color: "#2A2622" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>Langue</span>
              <span style={{ fontSize: 12, color: "#A99B7F" }}>Français</span>
            </div>
            <div style={{ fontSize: 11.5, color: "#A99B7F" }}>D'autres paramètres seront ajoutés prochainement.</div>
          </div>
        </MenuSection>

        <div style={{ padding: "18px 20px", borderTop: "1px solid #DCD1B8", marginTop: "auto" }}>
          {agent ? (
            ...
            <button type="button" onClick={onOpenLogin} style={{ ...primaryBtn, width: "100%", justifyContent: "center" }}>
              <Lock size={15} /> Espace inspecteurs
            </button>
          )}
        </div>
      </div>
...
function PasswordChangeForm({ accessToken, onDone, onCancel }) {
  ...
  const submit = async () => {
    setError("");
    if (!oldPwd) { setError("Veuillez saisir votre ancien mot de passe."); return; }
    if (!strong) { setError("Le nouveau mot de passe doit atteindre le niveau «Fort»."); return; }
    if (newPwd !== confirm) { setError("La confirmation ne correspond pas au nouveau mot de passe."); return; }
    setLoading(true);
    try {
      ...
      if (status === "incorrect") {
        setError("L'ancien mot de passe saisi est incorrect.");
  ...
  return (
    <div style={{ marginTop: 6 }}>
      <label style={labelStyle}>Ancien mot de passe<RequiredMark /></label>
      <PasswordInput value={oldPwd} onChange={(e) => setOldPwd(e.target.value)} />

      <label style={{ ...labelStyle, marginTop: 14 }}>Nouveau mot de passe<RequiredMark /></label>
      <PasswordInput value={newPwd} onChange={(e) => setNewPwd(e.target.value)} />
      <PasswordStrengthBar password={newPwd} />

      <label style={{ ...labelStyle, marginTop: 14 }}>Confirmer le nouveau mot de passe<RequiredMark /></label>
      ...
      <button type="button" onClick={onCancel} style={{ width: "100%", marginTop: 10, background: "none", border: "none", color: "#5B5346", fontSize: 12.5, cursor: "pointer" }}>
        Annuler
      </button>
    </div>
  );
...
function ProfileLockScreen({ agent, onUnlock, onCancel }) {
  ...
  return (
    ...
    >
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 340, padding: "32px 26px", borderRadius: 6, textAlign: "center" }}>
        <div
          style={{
            width: 48, height: 48, borderRadius: "50%", background: "#fff", border: "1px solid #DCD1B8",
            display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px",
          }}
        >
          <Lock size={20} color="#C9962C" />
        </div>
        <div style={{ fontFamily: "'Fraunces', serif", fontSize: 19, color: "#1C2B39", marginBottom: 6 }}>
          Profil verrouillé
        </div>
        <div style={{ fontSize: 12.5, color: "#5B5346", marginBottom: 20, lineHeight: 1.5 }}>
          Pour protéger vos informations personnelles, ressaisissez votre mot de passe pour continuer.
        </div>
        ...
        >
          <Lock size={14} /> {loading ? "Vérification..." : "Déverrouiller"}
        </button>
        <button
          type="button"
          onClick={onCancel}
          style={{ width: "100%", marginTop: 10, background: "none", border: "none", color: "#5B5346", fontSize: 12.5, cursor: "pointer" }}
        >
          ← Annuler
        </button>
      </div>
...
function ProfileEdit({ agent, accessToken, setAgent, onClose, onLogoutToLogin, onLockNow }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 110, background: "#1C2B39", overflowY: "auto", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 460, margin: "0 auto", padding: "30px 26px", borderRadius: 6, position: "relative" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <button
              ...
              title="Verrouiller le profil maintenant"
              style={{ display: "flex", alignItems: "center", gap: 5, background: "none", border: "1px solid #DCD1B8", borderRadius: 3, padding: "5px 9px", cursor: "pointer", color: "#5B5346", fontSize: 11.5 }}
            >
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 16, alignItems: "center" }}>
          <div
            style={{
              ...
              border: "1px solid #DCD1B8", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
            }}
          >
            {!form.photo && <Users size={22} color="#A99B7F" />}
          </div>
          <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
          <div>
            <label style={labelStyle}>Nom<RequiredMark /></label>
            <input style={inputStyle} value={form.nom} onChange={(e) => setForm({ ...form, nom: e.target.value })} />
          </div>
          <div>
            <label style={labelStyle}>Prénom<RequiredMark /></label>
            <input style={inputStyle} value={form.prenom} onChange={(e) => setForm({ ...form, prenom: e.target.value })} />
          </div>
          <div>
            <label style={labelStyle}>Date de naissance<RequiredMark /></label>
            <DateNaissancePicker value={form.dateNaissance} onChange={(v) => setForm({ ...form, dateNaissance: v })} />
          </div>
          <div>
            <label style={labelStyle}>Matricule<RequiredMark /></label>
            <input style={inputStyle} value={form.matricule} onChange={(e) => setForm({ ...form, matricule: e.target.value })} />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Adresse email<RequiredMark /></label>
            <input type="email" style={inputStyle} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
          </div>
        </div>

        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px dashed #DCD1B8" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <label style={{ ...labelStyle, marginBottom: 2 }}>Mot de passe</label>
              <div style={{ fontSize: 11.5, color: "#A99B7F" }}>
                {pwdJustChanged ? "Nouveau mot de passe défini ✓" : "••••••••"}
              </div>
            </div>
            {!pwdForm && (
              <button
              ...
              >
                <Pencil size={12} /> Modifier le mot de passe
              </button>
            )}
          </div>
        ...
        <div style={{ marginTop: 14, fontSize: 11.5, color: "#A99B7F" }}>
          Identifiant : <b style={{ color: "#5B5346" }}>{agent.login}</b>
        </div>

        {error && <div style={{ color: "#A8542E", fontSize: 12.5, marginTop: 14 }}>{error}</div>}
        {saved && <div style={{ color: "#4A5D3A", fontSize: 12.5, marginTop: 14 }}>Profil mis à jour.</div>}
        ...
        {pwdConfirm && (
          <div style={{ position: "absolute", inset: 0, background: "rgba(28,43,57,0.7)", borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
            <div style={{ background: "#fff", borderRadius: 6, padding: "22px 20px", maxWidth: 300, textAlign: "center" }}>
              <div style={{ fontSize: 14.5, color: "#1C2B39", fontWeight: 600, marginBottom: 18 }}>
                Voulez-vous modifier le mot de passe ?
              </div>
...
--------------------------------------------------------------- */
function Home({ onEnter, arrondissement, onOpenMenu, agent }) {
  const current = ARRONDISSEMENTS.find((a) => a.id === arrondissement) || ARRONDISSEMENTS[0];
  return (
    <div style={{ minHeight: "100vh", background: "#F1EBDD" }}>
      <NationalStripe />
      <header
        style={{
          background: "#1C2B39",
      ...
      >
        ...
        <div style={{ maxWidth: 760, margin: "0 auto", position: "relative" }}>
          ...
          >
            Délégation départementale de la Bénoué
            <span
              style={{
                display: "block",
                fontWeight: 400,
            ...
            >
              Contrôle des activités minières artisanales
            </span>
          </h1>
          <p
            style={{
          ...
          >
            Registre des artisans miniers, fiches de déclaration de terrain et
            délivrance des cartes professionnelles pour les sites aurifères
            dans le département de la Bénoué.
          </p>
          {agent ? (
            <div
              style={{
            ...
            >
              <CheckCircle2 size={15} color="#C9962C" /> Connecté — accédez au tableau de bord via le menu ☰
            </div>
          ) : (
            <button
              onClick={onEnter}
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "44px 24px 80px" }}>
          ...
          {[
            {
              ...
              title: "Fiches de déclaration",
              text: "Constats de terrain consignés par les inspecteurs assermentés.",
            },
            {
              icon: <Users size={20} />,
              title: "Registre des artisans",
              text: "Suivi des opérateurs et des quantités déclarées, site par site.",
            },
            {
              icon: <CreditCard size={20} />,
              title: "Cartes professionnelles",
              text: "Délivrance de la carte d'artisan minier après vérification.",
            },
          ].map((c) => (
            <div
              key={c.title}
            ...
            >
              <div style={{ color: "#1C2B39", marginBottom: 10 }}>{c.icon}</div>
              <div
                style={{
                  fontFamily: "'IBM Plex Sans', sans-serif",
                  fontWeight: 600,
        ...
        >
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14, color: "#2A2622", marginBottom: 6 }}>
              <MapPin size={15} color="#C9962C" /> Délégation départementale, Bénoué
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14, color: "#2A2622", marginBottom: 6 }}>
              <Phone size={15} color="#C9962C" /> +237 6XX XXX XXX
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14, color: "#2A2622" }}>
              <Mail size={15} color="#C9962C" /> contact@delegation-benoue.cm
            </div>
          </div>
        </section>
      </main>
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <button
        type="button"
        onClick={() => setShow((s) => !s)}
        style={{ position: "absolute", right: 6, top: 0, bottom: 0, background: "none", border: "none", cursor: "pointer", color: "#A99B7F", display: "flex", alignItems: "center" }}
      >
    ...
    <div style={{ position: "relative" }}>
      <button
        ...
        style={{
          ...
          textAlign: "left",
          cursor: "pointer",
          color: value && !value.startsWith("XXXX") ? "#2A2622" : "#A99B7F",
        }}
      >
      ...
      {open && (
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <span style={{ fontSize: 11.5, color: "#5B5346", fontFamily: "'IBM Plex Mono', monospace" }}>
              {step === "mois" && "1. Choisir le mois"}
              {step === "jour" && "2. Choisir le jour"}
              {step === "annee" && "3. Choisir l'année"}
            </span>
            <button type="button" onClick={() => setOpen(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "#A99B7F" }}>
              <X size={14} />
            </button>
          </div>
...
function PasswordStrengthBar({ password }) {
  ...
  return (
    <div style={{ marginTop: 6 }}>
      <div style={{ height: 5, background: "#E3DAC6", borderRadius: 3, overflow: "hidden" }}>
...
function ForgotPasswordModal({ onClose }) {
  ...
  const submitEmail = async () => {
    ...
    try {
      ...
      if (msg.includes("no_account")) {
        setError("Aucun compte inspecteur n'est associé à cette adresse email.");
      } else {
        setError("Une erreur est survenue. Réessayez.");
      }
    }
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(28,43,57,0.65)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 380, borderRadius: 6, padding: "26px 24px" }}>
        {step === "email" ? (
          <>
            <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 19, color: "#1C2B39", margin: "0 0 6px" }}>
              Mot de passe oublié
            </h3>
            <p style={{ fontSize: 12.5, color: "#5B5346", margin: "0 0 16px" }}>
              Saisissez votre adresse email pour recevoir un mot de passe temporaire.
            </p>
            <label style={{ display: "block", fontSize: 12.5, color: "#5B5346", marginBottom: 5 }}>Adresse email</label>
            <input
              type="email"
              value={email}
            ...
            <button type="button" onClick={submitEmail} disabled={loading} style={{ ...primaryBtn, width: "100%", marginTop: 18, justifyContent: "center", opacity: loading ? 0.6 : 1 }}>
              {loading ? "Génération..." : "Générer un mot de passe"}
            </button>
            <button type="button" onClick={onClose} style={{ width: "100%", marginTop: 10, background: "none", border: "none", color: "#5B5346", fontSize: 12.5, cursor: "pointer" }}>
              Annuler
            </button>
          </>
        ) : (
          <>
            ...
            <p style={{ fontSize: 12.5, color: "#5B5346", margin: "0 0 16px", lineHeight: 1.5 }}>
              Utilisez ce mot de passe pour vous connecter une seule fois. Il vous sera ensuite demandé d'en créer un nouveau.
            </p>
            <div
              style={{
                fontFamily: "'IBM Plex Mono', monospace",
            ...
            {copied && <div style={{ color: "#4A5D3A", fontSize: 11.5, textAlign: "center", marginBottom: 10 }}>Copié !</div>}
...
function ForceNewPasswordScreen({ email, accessToken, onDone }) {
  ...
  const validate = async () => {
    setError("");
    if (!strong) { setError("Le mot de passe doit atteindre le niveau «Fort»."); return; }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1C2B39", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 380, borderRadius: 6, padding: "30px 26px", textAlign: intro ? "center" : "left" }}>
        {intro ? (
          <>
            <Lock size={26} color="#C9962C" style={{ marginBottom: 12 }} />
            <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 19, color: "#1C2B39", margin: "0 0 10px" }}>
              Créer un nouveau mot de passe
            </h3>
            <p style={{ fontSize: 12.5, color: "#5B5346", lineHeight: 1.6, marginBottom: 20 }}>
              Ce mot de passe temporaire ne peut être utilisé qu'une seule fois. Vous devez en créer un nouveau pour accéder au tableau de bord.
            </p>
          ...
          <>
            <h3 style={{ fontFamily: "'Fraunces', serif", fontSize: 19, color: "#1C2B39", margin: "0 0 18px" }}>
              Nouveau mot de passe
            </h3>
            <label style={labelStyle}>Nouveau mot de passe<RequiredMark /></label>
            <PasswordInput value={newPassword} onChange={(e) => setNewPassword(e.target.value)} />
            <PasswordStrengthBar password={newPassword} />
            <label style={{ ...labelStyle, marginTop: 14 }}>Confirmer le mot de passe<RequiredMark /></label>
            <PasswordInput value={confirm} onChange={(e) => setConfirm(e.target.value)} />
            {error && <div style={{ color: "#A8542E", fontSize: 12.5, marginTop: 12 }}>{error}</div>}
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (!rows || rows.length === 0) {
        setError("Aucun profil inspecteur associé à ce compte.");
        setLoading(false);
        return;
      }
      ...
      if (msg.includes("confirm")) {
        setError("Adresse e-mail non confirmée. Vérifiez votre boîte mail et cliquez sur le lien reçu avant de vous connecter.");
      } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 24, color: "#1C2B39", margin: "6px 0 22px" }}>
          Espace inspecteurs
        </h2>

        {justCreated && (
          <div style={{ background: "#EAF0E5", color: "#4A5D3A", fontSize: 12.5, padding: "8px 10px", borderRadius: 3, marginBottom: 16 }}>
            Compte créé avec succès. Connectez-vous avec votre email et votre mot de passe.
          </div>
        )}
        {justChangedPwd && (
          <div style={{ background: "#EAF0E5", color: "#4A5D3A", fontSize: 12.5, padding: "8px 10px", borderRadius: 3, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "16px 0 5px" }}>
          <label style={{ fontSize: 12.5, color: "#5B5346" }}>Mot de passe</label>
      ...
      {[0, 1, 2, 3].map((idx) => (
        <input
          ...
          style={{
            ...
            borderRadius: 4,
            background: disabled ? "#EDE6D4" : "#fff",
            color: "#1C2B39",
          }}
        />
      ))}
...
function Register({ onBack, onCreated }) {
  ...
  const requestCode = async () => {
    ...
    try {
      ...
      setOtpError("Impossible d'envoyer le code pour le moment. Réessayez dans un instant.");
      return false;
    }
  };
  ...
  const submitForm = async () => {
    ...
    const requiredOk =
      ...
      form.photo;
    if (!requiredOk) {
      setFormError("Veuillez renseigner tous les champs obligatoires (*), y compris la photo.");
      return;
    }
    if (!strong) {
      setFormError("Le mot de passe doit atteindre le niveau «Fort» (barre verte) avant de continuer.");
      return;
    }
    if (password !== confirm) {
      setFormError("La confirmation ne correspond pas au mot de passe.");
      return;
    }
    setLoading(true);
    if (USE_CUSTOM_OTP) {
  ...
  if (step === "confirmLink") {
    return (
      <div style={{ minHeight: "100vh", background: "#1C2B39", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 400, padding: "34px 28px", borderRadius: 6, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: "#2A2622", lineHeight: 1.6, margin: "0 0 6px" }}>
            Un e-mail de confirmation a été envoyé à :
          </p>
          <p style={{ fontSize: 14, color: "#1C2B39", fontWeight: 700, margin: "0 0 16px" }}>{form.email}</p>
          <p style={{ fontSize: 13, color: "#5B5346", lineHeight: 1.6, margin: "0 0 22px" }}>
  ...
  if (step === "otp") {
    return (
      <div style={{ minHeight: "100vh", background: "#1C2B39", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 400, padding: "30px 26px", borderRadius: 6, textAlign: "center" }}>
          ...
          <p style={{ fontSize: 14, color: "#2A2622", lineHeight: 1.5, margin: "0 0 4px" }}>
            Un code a été envoyé à votre adresse mail :
          </p>
          <p style={{ fontSize: 14, color: "#1C2B39", fontWeight: 700, margin: "0 0 10px" }}>{form.email}</p>
          <p style={{ fontSize: 12.5, color: "#5B5346", margin: 0 }}>Veuillez le saisir ci-dessous.</p>
          ...
          {otpLoading && (
            <div style={{ color: "#5B5346", fontSize: 12.5, marginBottom: 14 }}>{showRetryFinalize ? "Nouvelle tentative..." : "Vérification..."}</div>
          )}
          ...
          {showRetryFinalize ? (
            ...
            >
              {otpLoading ? "Nouvelle tentative..." : "Réessayer"}
            </button>
          ) : (
            <button
              type="button"
            ...
            >
              {regenDisabled ? `Générer un code (${countdown}s)` : "Générer un code"}
            </button>
          )}
          ...
          <button
            ...
            style={{ width: "100%", marginTop: 14, background: "none", border: "none", color: "#5B5346", fontSize: 12.5, cursor: "pointer" }}
          >
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1C2B39", padding: "32px 16px 60px", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <div style={{ background: "#F1EBDD", width: "100%", maxWidth: 460, margin: "0 auto", padding: "30px 26px", borderRadius: 6 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#C9962C", marginBottom: 4 }}>
          <Stamp size={18} />
          <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, letterSpacing: "0.12em", textTransform: "uppercase" }}>
            Nouveau compte inspecteur
          </span>
        </div>
        <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 22, color: "#1C2B39", margin: "6px 0 22px" }}>
          Créer un compte
        </h2>
        ...
        <div style={{ display: "flex", gap: 14, marginBottom: 16, alignItems: "center" }}>
          <div
            style={{
              ...
              borderRadius: 6,
              background: form.photo ? `url(${form.photo}) center/cover` : "#fff",
              border: form.photo ? "1px solid #DCD1B8" : "1.5px solid #A8542E",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              flexShrink: 0,
          ...
          <div>
            <label style={labelStyle}>Photo<RequiredMark /></label>
            <input type="file" accept="image/*" onChange={handlePhoto} style={{ fontSize: 12.5 }} />
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
          ...
          <div>
            <label style={labelStyle}>Matricule<RequiredMark /></label>
            <input style={inputStyle} placeholder="Ex : BEN/2026/014" value={form.matricule} onChange={(e) => setForm({ ...form, matricule: e.target.value })} />
          </div>
          <div style={{ gridColumn: "1 / -1" }}>
            <label style={labelStyle}>Adresse email<RequiredMark /></label>
            <input type="email" style={inputStyle} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
        ...
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px dashed #DCD1B8" }}>
          ...
          <div style={{ fontSize: 11, color: "#A99B7F", marginTop: 5, lineHeight: 1.5 }}>
        ...
        <button type="button" onClick={submitForm} disabled={loading} style={{ ...primaryBtn, width: "100%", marginTop: 20, justifyContent: "center", opacity: loading ? 0.6 : 1 }}>
          {loading ? "Envoi du code..." : "Valider la création du compte"}
        </button>
        <button
          type="button"
          onClick={onBack}
...
/* ---------------------------------------------------------------
   Dashboard
--------------------------------------------------------------- */
function Dashboard({ agent, accessToken, artisans, setArtisans, controles, setControles, cartes, setCartes, arrondissement, onOpenMenu, jumpTab }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F1EBDD", fontFamily: "'IBM Plex Sans', sans-serif" }}>
      <NationalStripe />
      <div style={{ background: "#1C2B39", color: "#F1EBDD" }}>
        ...
        <div style={{ maxWidth: 980, margin: "0 auto", padding: "0 20px", display: "flex", gap: 4, overflowX: "auto" }}>
          {[
            ...
            { id: "operateurs", label: "Artisans", icon: <Users size={14} /> },
            { id: "controle", label: "Fiche de déclaration", icon: <ClipboardList size={14} /> },
            { id: "cartes", label: "Cartes", icon: <CreditCard size={14} /> },
            { id: "qrcode", label: "QR code", icon: <QrCode size={14} /> },
            { id: "agents", label: "Inspecteurs", icon: <Lock size={14} /> },
          ].map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px 60px" }}>
        {tab === "stats" && <Statistiques artisans={artisans} controles={controles} />}
        {tab === "operateurs" && <Operateurs artisans={artisans} setArtisans={setArtisans} arrondissement={arrondissement} accessToken={accessToken} agent={agent} />}
...
function StatBar({ label, value, total, color }) {
  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, color: "#2A2622", marginBottom: 4 }}>
        <span>{label}</span>
        <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: "#5B5346" }}>{value} ({pct}%)</span>
      </div>
      <div style={{ height: 8, background: "#E3DAC6", borderRadius: 4, overflow: "hidden" }}>
...
function StatCard({ icon, value, label, color }) {
  return (
    <div style={{ ...cardStyle, display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <div>
        <div style={{ fontFamily: "'Fraunces', serif", fontSize: 22, color: "#1C2B39", lineHeight: 1 }}>{value}</div>
        <div style={{ fontSize: 12, color: "#5B5346", marginTop: 2 }}>{label}</div>
      </div>
    </div>
  );
}

function Statistiques({ artisans, controles }) {
  ...
  const cartesExpirees = artisans.filter((a) => statutExpiration(a.dateExpiration) === "expiree");
  const cartesARenouveler = artisans.filter((a) => statutExpiration(a.dateExpiration) === "bientot");
  ...
  declarations.forEach((c) => {
    const commune = c.commune || "Commune non précisée";
  ...
  const parSite = {};
  artisans.forEach((a) => {
    const site = a.site?.trim() || "Site non précisé";
  ...
  return (
    <div>
      <SectionHeader title="Statistiques" subtitle="Vue d'ensemble du registre et des déclarations" />

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 22 }}>
        <StatCard icon={<Users size={18} color="#1C2B39" />} value={totalArtisans} label="Artisans enregistrés" color="#1C2B39" />
        <StatCard icon={<CheckCircle2 size={18} color="#4A5D3A" />} value={enRegle} label="En règle" color="#4A5D3A" />
        <StatCard icon={<AlertTriangle size={18} color="#A8542E" />} value={nonRegle} label="Non en règle" color="#A8542E" />
        <StatCard icon={<ClipboardList size={18} color="#C9962C" />} value={totalDeclarations} label="Fiches de déclaration" color="#C9962C" />
        <StatCard icon={<CalendarClock size={18} color="#A8542E" />} value={cartesExpirees.length} label="Cartes expirées" color="#A8542E" />
        <StatCard icon={<CalendarClock size={18} color="#8A6416" />} value={cartesARenouveler.length} label={`À renouveler (${SEUIL_RENOUVELLEMENT_JOURS} j)`} color="#8A6416" />
      </div>

      {cartesAlerte.length > 0 && (
        <div style={{ ...cardStyle, marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 14 }}>Cartes à traiter en priorité</div>
            ...
            {cartesAlerte.slice(0, 8).map((a) => {
              ...
              return (
                <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, fontSize: 12.5 }}>
                  <span style={{ color: "#1C2B39" }}>{a.prenom ? `${a.prenom} ${a.nom}` : a.nom}</span>
                  <span style={{ color: expiree ? "#A8542E" : "#8A6416", fontFamily: "'IBM Plex Mono', monospace", fontSize: 11.5, whiteSpace: "nowrap" }}>
                    {expiree ? `expirée depuis ${Math.abs(jours)} j` : `dans ${jours} j`}
                  </span>
                </div>
              );
            })}
      ...
      <div style={cardStyle}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 14 }}>Artisans par commune</div>
        {parCommune.map((c) => (
          <StatBar
            key={c.id}
            label={c.label}
        ...
        {parCommune.length === 0 && <div style={{ fontSize: 12.5, color: "#A99B7F" }}>Aucune commune avec artisans enregistrés et déclarés pour l'instant.</div>}
      </div>

      <div style={{ ...cardStyle, marginTop: 14 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 14 }}>Conformité du registre</div>
        <StatBar label="Artisans en règle" value={enRegle} total={totalArtisans} color="#4A5D3A" />
        <StatBar label="Artisans non en règle" value={nonRegle} total={totalArtisans} color="#A8542E" />
        {totalArtisans === 0 && <div style={{ fontSize: 12.5, color: "#A99B7F" }}>Aucun artisan enregistré pour l'instant.</div>}
      </div>

      <div style={{ ...cardStyle, marginTop: 14 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 14 }}>Taxe totale déclarée</div>
        <div style={{ fontFamily: "'Fraunces', serif", fontSize: 28, color: "#1C2B39" }}>{taxeTotaleCollectee.toLocaleString("fr-FR")} {totalDeclarations > 0 ? derniereDevise : ""}</div>
      </div>

      <div style={{ ...cardStyle, marginTop: 14 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 4 }}>Quantités totales déclarées par commune</div>
        <div style={{ fontFamily: "'Fraunces', serif", fontSize: 22, color: "#1C2B39", marginBottom: 14 }}>{formatQuantite(quantiteTotaleDeclaree)}</div>
        {communesDeclarees.length === 0 ? (
          <div style={{ fontSize: 12.5, color: "#A99B7F" }}>Aucune déclaration enregistrée pour l'instant.</div>
        ) : (
      ...
      {topSites.length > 0 && (
        <div style={{ ...cardStyle, marginTop: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 14 }}>Sites les plus actifs</div>
...
function statutExpiration(dateExpirationISO) {
  const jours = joursAvantExpiration(dateExpirationISO);
  if (jours === null) return null;
  if (jours < 0) return "expiree";
  if (jours <= SEUIL_RENOUVELLEMENT_JOURS) return "bientot";
  return null;
}
...
function GpsField({ value, onChange }) {
  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 10.5, color: "#A99B7F", marginTop: 4, lineHeight: 1.4 }}>
...
const SUBSTANCE_CATEGORIES = {
  "Minéraux précieux": ["Or", "Diamant", "Saphir"],
  "Minéraux semi-précieux": ["Quartz gemme", "Grenat", "Tourmaline"],
  "Métaux de base": ["Wolframite", "Cassitérite", "Bauxite", "Rutile", "Cuivre", "Plomb", "Zinc", "Manganèse"],
  "Matériaux de carrière": ["Sable", "Gravier", "Galet", "Latérite", "Argile", "Calcaire", "Granite", "Grès", "Ferrugineux"],
};
const SUBSTANCE_CATEGORIE_KEYS = Object.keys(SUBSTANCE_CATEGORIES);

function Operateurs({ artisans, setArtisans, arrondissement, accessToken, agent }) {
  ...
  const [form, setForm] = useState({
    ...
    statut: "en_regle",
    photo: "",
    substanceCategorie: "Minéraux précieux",
    substance: "Or",
  ...
  const addArtisan = (e) => {
    ...
    const requiredOk =
      ...
      form.photo;
    if (!requiredOk) {
      setError("Veuillez renseigner tous les champs obligatoires (*), y compris la photo.");
      return;
    }
  ...
  const removeArtisan = (id) => {
    if (!window.confirm("Supprimer cet artisan du registre ?")) return;
  ...
  return (
    <div>
      <SectionHeader
        title="Registre des artisans miniers"
        ...
        action={
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", position: "relative" }}>
            ...
            <button style={primaryBtn} onClick={() => setShowForm((s) => !s)}>
              {showForm ? <X size={14} /> : <Plus size={14} />} {showForm ? "Fermer" : "Ajouter un artisan"}
            </button>
            ...
            {showListe && (
              ...
              >
                {artisans.length === 0 && (
                  <div style={{ padding: 14, fontSize: 12.5, color: "#A99B7F" }}>Aucun artisan enregistré.</div>
                )}
                {artisans.map((a) => (
                  <button
                    key={a.id}
      ...
      {showForm && (
        <div style={cardStyle}>
          <div style={{ display: "flex", gap: 16, marginBottom: 14, alignItems: "center", flexWrap: "wrap" }}>
            <div
              style={{
                ...
                height: 72,
                borderRadius: 6,
                background: form.photo ? `url(${form.photo}) center/cover` : "#F1EBDD",
                border: form.photo ? "1px solid #DCD1B8" : "1.5px solid #A8542E",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
            ...
            <div>
              <label style={labelStyle}>Photo de l'artisan<RequiredMark /></label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
            ...
            <div>
              <label style={labelStyle}>N° CNI<RequiredMark /></label>
            ...
            <div>
              <label style={labelStyle}>Site d'exploitation</label>
            ...
            <div>
              <label style={labelStyle}>Zone de prospection</label>
              <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value="Bénoué" readOnly />
            </div>
            <div>
              <label style={labelStyle}>Type de substance à exploiter</label>
              <select
                style={inputStyle}
            ...
            <div>
              <label style={labelStyle}>Substance précise</label>
            ...
            <div>
              <label style={labelStyle}>Date de délivrance de la carte<RequiredMark /></label>
              <DateNaissancePicker value={form.dateDelivrance} onChange={(v) => setForm({ ...form, dateDelivrance: v })} />
            </div>
            <div>
              <label style={labelStyle}>Date d'expiration de la carte</label>
              <input
                style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }}
          ...
          <div style={{ fontSize: 11, color: "#A99B7F", marginTop: 10 }}>
            La carte est valable 2 ans à partir de la date de délivrance. Le numéro de carte (format CAM-Année-Commune-N°) est généré automatiquement à l'enregistrement.
          </div>

          {error && <div style={{ color: "#A8542E", fontSize: 12.5, marginTop: 10 }}>{error}</div>}

          <button type="button" onClick={addArtisan} style={{ ...primaryBtn, marginTop: 16 }}>Enregistrer</button>
        </div>
      )}

      <div style={{ display: "flex", gap: 10, margin: "18px 0 14px", flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 180 }}>
          <Search size={15} style={{ position: "absolute", left: 12, top: 11, color: "#A99B7F" }} />
          <input
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {filtered.length === 0 && <EmptyState text="Aucun artisan enregistré pour le moment." />}
        {filtered.map((a) => (
          <div
            key={a.id}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div
                style={{
                  ...
                  height: 40,
                  borderRadius: 5,
                  background: a.photo ? `url(${a.photo}) center/cover` : "#F1EBDD",
                  border: "1px solid #E3DAC6",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
              ...
              >
                {!a.photo && <Users size={16} color="#A99B7F" />}
              </div>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14.5, color: "#1C2B39" }}>
                  {a.prenom ? `${a.prenom} ${a.nom}` : a.nom}
                </div>
                <div style={{ fontSize: 12.5, color: "#5B5346", fontFamily: "'IBM Plex Mono', monospace" }}>
                  {a.numeroCarte ? `${a.numeroCarte} · ` : ""}{a.site || "site non précisé"} · CNI {a.cni || "—"}
                </div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <ExpirationBadge dateExpiration={a.dateExpiration} />
...
function ArtisanProfile({ artisan, artisans, setArtisans, onClose, accessToken, agent }) {
  ...
  const [editForm, setEditForm] = useState({
    ...
    substanceCategorie: artisan.substanceCategorie || "Minéraux précieux",
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: "#F1EBDD", width: "100%", maxWidth: 440, borderRadius: 6, overflow: "hidden", position: "relative" }}
      >
        <div style={{ background: "#1C2B39", padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <span style={{ color: "#C9962C", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "'IBM Plex Mono', monospace" }}>
            {mode === "edit" ? "Modifier le profil" : "Profil de l'artisan"}
          </span>
          {mode !== "confirm" && (
            <button onClick={onClose} style={{ background: "none", border: "none", color: "#F1EBDD", cursor: "pointer" }}>
              <X size={18} />
            </button>
          )}
        </div>
        ...
        <div style={{ padding: "22px 20px" }}>
          <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 18 }}>
            ...
            >
              {!(mode === "edit" ? editForm.photo : artisan.photo) && <Users size={24} color="#A99B7F" />}
            </div>
            <div>
              <div style={{ fontFamily: "'Fraunces', serif", fontSize: 19, color: "#1C2B39" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                ...
                <ExpirationBadge dateExpiration={artisan.dateExpiration} />
              </div>
              {mode === "edit" && (
                <div style={{ marginTop: 6 }}>
          ...
          {mode !== "edit" ? (
            <div style={{ display: "grid", gap: 8, fontSize: 13.5, color: "#2A2622" }}>
              <ProfileRow label="N° de carte" value={artisan.numeroCarte || "—"} />
              <ProfileRow label="Date de naissance" value={formatDateFR(artisan.dateNaissance) || "—"} />
              <ProfileRow label="Lieu de naissance" value={artisan.lieuNaissance || "—"} />
              <ProfileRow label="N° CNI" value={artisan.cni || "—"} />
              <ProfileRow label="Téléphone" value={artisan.telephone || "—"} />
              <ProfileRow label="Site d'exploitation" value={artisan.site || "—"} />
              <ProfileRow
                ...
                value={
                  artisan.gpsSite ? (
                    <button
                      ...
                      style={{ background: "none", border: "none", padding: 0, cursor: "pointer", color: "#1C2B39", fontWeight: 600, textDecoration: "underline", textDecorationColor: "#C9962C", fontFamily: "'IBM Plex Sans', sans-serif", fontSize: "inherit" }}
                    >
                      {artisan.gpsSite}
                    </button>
                  ) : "—"
              ...
              />
              <ProfileRow label="Zone de prospection" value={artisan.zoneProspection || "Bénoué"} />
              <ProfileRow label="Catégorie de substance" value={artisan.substanceCategorie || "—"} />
              <ProfileRow label="Substance exploitée" value={artisan.substance || "—"} />
              <ProfileRow label="Commune" value={commune?.commune || "—"} />
              <ProfileRow label="Date de délivrance" value={formatDateFR(artisan.dateDelivrance) || "—"} />
              <ProfileRow label="Date d'expiration" value={formatDateFR(artisan.dateExpiration) || "—"} />
              <div style={{ marginTop: 6, paddingTop: 10, borderTop: "1px dashed #DCD1B8", fontSize: 12, color: "#5B5346", lineHeight: 1.6 }}>
                <div>Créé par <b style={{ color: "#1C2B39" }}>{artisan.creePar || "—"}</b>{artisan.creeLe ? ` le ${formatDateTimeFR(artisan.creeLe)}` : ""}</div>
                {(artisan.modifiePar && artisan.modifieLe && artisan.modifieLe !== artisan.creeLe) && (
                  <div>Dernière modification par <b style={{ color: "#1C2B39" }}>{artisan.modifiePar}</b> le {formatDateTimeFR(artisan.modifieLe)}</div>
                )}
              </div>
            </div>
          ) : (
            <div style={{ display: "grid", gap: 12 }}>
              <div style={{ fontSize: 11, color: "#A99B7F" }}>
                Les informations d'identité (nom, prénom, date de naissance, CNI, n° de carte) ne sont pas modifiables ici.
              </div>
              <div>
                <label style={labelStyle}>Lieu de naissance</label>
              ...
              <div>
                <label style={labelStyle}>Coordonnées GPS</label>
          ...
          {mode === "view" && (
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 22 }}>
              <button type="button" onClick={onClose} style={primaryBtn}>Enregistrer</button>
          ...
          {mode === "edit" && (
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 20 }}>
              <button type="button" onClick={saveChanges} style={primaryBtn}>Enregistrer</button>
            </div>
          )}
        </div>
          ...
          >
            <div style={{ background: "#fff", borderRadius: 6, padding: "22px 20px", maxWidth: 320, textAlign: "center" }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 18, justifyContent: "center" }}>
                <button type="button" onClick={() => setMode("edit")} style={{ ...primaryBtn, minWidth: 70, justifyContent: "center" }}>Oui</button>
                <button type="button" onClick={() => setMode("view")} style={{ ...ghostBtn, minWidth: 70, justifyContent: "center" }}>Non</button>
              </div>
            </div>
          </div>
        )}
...
function ProfileRow({ label, value }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, borderBottom: "1px solid #E3DAC6", paddingBottom: 6 }}>
      <span style={{ color: "#5B5346" }}>{label}</span>
...
function ExpirationBadge({ dateExpiration }) {
  const statut = statutExpiration(dateExpiration);
...
const TAUX_PAR_CATEGORIE = {
  "Minéraux précieux": 5,
  "Minéraux semi-précieux": 5,
  "Métaux de base": 3,
  "Matériaux de carrière": 3,
};
...
function ApercuRow({ label, value, bold }) {
  return (
    <tr style={{ borderBottom: "1px solid #E3DAC6" }}>
      <td style={{ padding: "4px 8px 4px 0", color: "#5B5346", width: "50%" }}>{label}</td>
      <td style={{ padding: "4px 0", fontWeight: bold ? 700 : 600, color: "#1C2B39" }}>{value}</td>
    </tr>
  );
}
...
function DeclarationApercu({ record: c }) {
  ...
  return (
    <div className="print-area" style={{ ...cardStyle, marginBottom: 18, maxWidth: 800, width: "100%", margin: "0 auto 18px" }}>
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: "#1C2B39", marginBottom: 14, textAlign: "center" }}>Fiche de déclaration — {c.mois}</div>

      <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", marginBottom: 6 }}>1. Identification de l'exploitation</div>
      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11, marginBottom: 16 }}>
        <tbody>
          <ApercuRow label="Nom ou raison sociale" value={c.operateur} />
          <ApercuRow label="Adresse / Tél" value={c.adresseTel || "—"} />
          <ApercuRow label="N° de contribuable" value={c.numeroContribuable || "—"} />
          <ApercuRow label="Référence titre minier" value={c.referenceTitre || "—"} />
          <ApercuRow label="Titre minier" value={c.titreMinier} />
          <ApercuRow label="Localisation du site" value={c.localisation || "—"} />
          <ApercuRow label="Département" value={c.departement} />
          <ApercuRow label="Arrondissement" value={c.arrondissement} />
          <ApercuRow label="Commune" value={c.commune} />
        </tbody>
      </table>

      <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", marginBottom: 6 }}>2. Quantité produite et taxe ad valorem</div>
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 16, tableLayout: "fixed" }}>
        <tbody>
          <tr>
            <td style={th}>Désignation</td>
            <td style={th}>Quantité déclarée ({c.unite})</td>
            <td style={th}>Taux applicable ({c.taux}%)</td>
            <td style={th}>Montant de la taxe</td>
          </tr>
          <tr>
      ...
      <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", marginBottom: 6 }}>3. Répartition</div>
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 16, tableLayout: "fixed" }}>
        <tbody>
          <tr>
            <td style={th}>Bénéficiaire</td>
            <td style={th}>Taux applicable</td>
            <td style={th}>Montant</td>
            <td style={th}>Compte d'affectation</td>
          </tr>
          <tr>
            <td style={td}>CTD</td>
            <td style={td}>Commune (25%)</td>
          ...
          <tr>
            <td style={td}>Trésor public</td>
            <td style={td}>Trésor public (65%)</td>
      ...
      <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", marginBottom: 6 }}>4. Documents présentés</div>
      <div style={{ fontSize: 12.5, marginBottom: 16 }}>
        {c.documents?.autorisation && <div>✓ Autorisation d'exploitation artisanale</div>}
        {c.documents?.registre && <div>✓ Registre de production</div>}
        {c.documents?.quittance && <div>✓ Quittance de paiement des taxes</div>}
        {!c.documents?.autorisation && !c.documents?.registre && !c.documents?.quittance && (
          <div style={{ color: "#A99B7F" }}>Aucun document coché</div>
        )}
      </div>

      <div style={{ fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", marginBottom: 6 }}>5. Observations</div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 30, padding: "0 16px 10px" }}>
        <div style={{ textAlign: "center", fontSize: 11, color: "#1C2B39" }}>
          <div style={{ marginBottom: 34 }}>Signature de l'exploitant</div>
        </div>
        <div style={{ textAlign: "center", fontSize: 11, color: "#1C2B39" }}>
          <div style={{ marginBottom: 34 }}>Visa du délégué départemental</div>
        </div>
      </div>
      ...
      <div className="no-print" style={{ textAlign: "center", fontSize: 11, color: "#A99B7F", marginTop: 6, lineHeight: 1.6 }}>
        ...
        {c.modifie && c.modifiePar && c.modifieLe && (
          <> · Dernière modification par <b style={{ color: "#5B5346" }}>{c.modifiePar}</b> le {formatDateTimeFR(c.modifieLe)}</>
        )}
      </div>
    </div>
  );
...
function FicheControle({ artisans, controles, setControles, agent, accessToken }) {
  ...
  const removeDeclaration = (id) => {
    if (!window.confirm("Supprimer définitivement cette fiche de déclaration ?")) return;
  ...
  return (
    <div>
      ...
      `}</style>
      <SectionHeader
        title="Fiche de déclaration de terrain"
        subtitle="Déclarations mensuelles consignées par artisan"
        action={
          <div className="no-print" style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button type="button" onClick={() => { setShowWizard(true); setWizStep("select"); setShowRepertoire(false); }} style={primaryBtn}>
              <ClipboardList size={14} /> Enregistrer une fiche de déclaration
            </button>
            <button type="button" onClick={() => { setShowRepertoire((s) => !s); setShowWizard(false); }} style={ghostBtn}>
              <Search size={14} /> Déclaration
            </button>
            {declarations.length > 0 && (
              <button type="button" onClick={exportCsv} style={ghostBtn}>
                <Printer size={14} /> Exporter en CSV
              </button>
            )}
          </div>
        }
      ...
      {showRepertoire && (
        <div className="no-print" style={{ ...cardStyle, marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 12 }}>Consulter une déclaration</div>
          ...
          {artisansAvecDeclaration.length === 0 && (
            <div style={{ fontSize: 12.5, color: "#A99B7F", marginTop: 10 }}>Aucune déclaration enregistrée pour l'instant.</div>
          )}
          <button type="button" onClick={fermerRepertoire} style={{ ...ghostBtn, marginTop: 16 }}>Fermer</button>
        </div>
      )}
      ...
      {showRepertoire && repResult && (
        <>
          ...
          <div className="no-print" style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 18 }}>
            ...
            <button type="button" onClick={fermerRepertoire} style={ghostBtn}>Fermer</button>
          </div>
        </>
      )}
      ...
      {showWizard && wizStep === "select" && (
        <div className="no-print" style={{ ...cardStyle, marginBottom: 18 }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "#1C2B39", marginBottom: 12 }}>Nouvelle déclaration — sélection</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12 }}>
            <div>
              <label style={labelStyle}>Artisan enregistré</label>
          ...
          {artisans.length === 0 && (
            <div style={{ fontSize: 12.5, color: "#A8542E", marginTop: 10 }}>Le registre est vide — ajoutez d'abord un artisan dans l'onglet Artisans.</div>
          )}
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <button type="button" onClick={validerSelection} style={primaryBtn} disabled={!selArtisanId}>Valider</button>
            <button type="button" onClick={resetWizard} style={ghostBtn}>Annuler</button>
          </div>
        </div>
      )}
        ...
        <div className="no-print" style={{ display: "grid", gap: 14, marginBottom: 18 }}>
          <div style={cardStyle}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
              <div>
                <label style={labelStyle}>Mois</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={moisLabel} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Nom ou raison sociale de l'exploitant</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={artisanSel.prenom ? `${artisanSel.prenom} ${artisanSel.nom}` : artisanSel.nom} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Adresse / Tél</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={artisanSel.telephone || "—"} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Numéro de contribuable</label>
                <input style={inputStyle} value={decl.numeroContribuable} onChange={(e) => setDecl({ ...decl, numeroContribuable: e.target.value })} />
              </div>
              <div>
                <label style={labelStyle}>Référence du titre minier</label>
                <input style={inputStyle} value={decl.referenceTitre} onChange={(e) => setDecl({ ...decl, referenceTitre: e.target.value })} />
              </div>
              <div>
                <label style={labelStyle}>Titre minier</label>
              ...
              <div>
                <label style={labelStyle}>Localisation du site</label>
              ...
              <div>
                ...
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={communeSel?.label || "—"} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Commune</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={communeSel?.commune || "—"} readOnly />
              </div>
            </div>
          </div>
          ...
          <div style={cardStyle}>
            <div style={{ fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase", color: "#C9962C", fontFamily: "'IBM Plex Mono', monospace", marginBottom: 12 }}>
              2. Quantité produite et taxe ad valorem
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
              <div>
                <label style={labelStyle}>Désignation de la substance</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={artisanSel.substance || "—"} readOnly />
              </div>
              <div>
              ...
              <div>
                <label style={labelStyle}>Taux applicable</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={`${tauxSel} % (${artisanSel.substanceCategorie})`} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Valeur monétaire</label>
              ...
              <div>
                <label style={labelStyle}>Prix sur le marché</label>
                <input style={inputStyle} value={decl.montant} onChange={(e) => setDecl({ ...decl, montant: e.target.value })} placeholder="Ex : 5000" />
              </div>
              <div>
                <label style={labelStyle}>Montant de la taxe (par unité)</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={`${taxeUnitaire.toLocaleString("fr-FR")} ${decl.devise}`} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Montant total de la taxe</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#1C2B39", fontWeight: 700 }} value={`${taxeTotale.toLocaleString("fr-FR")} ${decl.devise}`} readOnly />
              </div>
            </div>
          </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
              <div>
                ...
                <select style={inputStyle} value={decl.beneficiaireType} onChange={(e) => setDecl({ ...decl, beneficiaireType: e.target.value })}>
                  <option value="CTD">CTD (Commune)</option>
                  <option value="appui">Appui suivi et contrôle des activités minières</option>
                  <option value="tresor">Trésor public</option>
                </select>
              </div>
              ...
              <div>
                <label style={labelStyle}>Taux applicable</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={`${beneficiaireInfo.taux} %`} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Compte d'affectation</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#5B5346" }} value={beneficiaireInfo.compte} readOnly />
              </div>
              <div>
                <label style={labelStyle}>Montant réparti</label>
                <input style={{ ...inputStyle, background: "#EDE6D4", color: "#1C2B39", fontWeight: 700 }} value={`${montantRepartition.toLocaleString("fr-FR")} ${decl.devise}`} readOnly />
              </div>
            </div>
          </div>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              {[
                ["autorisation", "Autorisation d'exploitation artisanale"],
                ["registre", "Registre de production"],
                ["quittance", "Quittance de paiement des taxes"],
              ].map(([key, label]) => (
                <label key={key} style={{ display: "flex", alignItems: "center", gap: 7, fontSize: 13, color: "#2A2622" }}>
                  <input
                    type="checkbox"
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button type="button" onClick={submitDeclaration} style={primaryBtn}>Enregistrer la déclaration</button>
            <button type="button" onClick={resetWizard} style={ghostBtn}>Annuler</button>
          </div>
        </div>
      )}
      ...
      {showWizard && wizStep === "apercu" && savedRecord && (
        <>
          ...
          <div className="no-print" style={{ display: "flex", gap: 10, justifyContent: "center", marginBottom: 18, flexWrap: "wrap" }}>
            ...
            <button type="button" onClick={() => removeDeclaration(savedRecord.id)} style={{ ...ghostBtn, color: "#A8542E", borderColor: "#E3B8A8" }}><Trash2 size={14} /> Supprimer</button>
            <button type="button" onClick={resetWizard} style={ghostBtn}>Fermer</button>
          </div>
        </>
      )}
      ...
      <div style={{ marginTop: 24 }} className="no-print">
        <div style={{ fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", color: "#C9962C", fontFamily: "'IBM Plex Mono', monospace", marginBottom: 10 }}>
          Historique de déclaration ({declarations.length})
        </div>
        <div style={{ display: "grid", gap: 10 }}>
          {declarations.length === 0 && <EmptyState text="Aucune déclaration enregistrée." />}
          {declarations.map((c) => (
            <div key={c.id} style={cardStyle}>
              <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                <div style={{ fontWeight: 600, color: "#1C2B39" }}>{c.operateur}</div>
                <span style={{ fontSize: 11.5, fontFamily: "'IBM Plex Mono', monospace", color: "#C9962C" }}>{c.mois}</span>
              </div>
              <div style={{ fontSize: 12.5, color: "#5B5346", marginTop: 4 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
                <div style={{ fontSize: 11, color: "#A99B7F" }}>
                  Créée par {c.agent || "—"}{c.modifie ? ` · modifiée par ${c.modifiePar || "—"}` : ""}
                </div>
                <div style={{ display: "flex", gap: 6 }}>
                  <button
                    type="button"
                    ...
                    title="Voir la fiche"
                    style={{ background: "none", border: "none", cursor: "pointer", color: "#5B5346" }}
                  >
                    <Eye size={15} />
                  </button>
                  <button
      ...
      <div className="carte-page" style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
          ...
          >
            ...
            <div style={{ textAlign: "center", marginTop: 3 }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 9, color: "#1C2B39", textTransform: "uppercase", lineHeight: 1.15 }}>Carte d'artisan minier</div>
            </div>
            ...
            <div style={{ display: "flex", gap: 7, marginTop: 5, flex: 1 }}>
              ...
              <div style={{ flex: 1, minWidth: 0, fontSize: 6.3, color: "#1C2B39", lineHeight: 1.55 }}>
                <div><u>NOM</u> : <b>{artisan.nom.toUpperCase()}</b></div>
                <div><u>PRENOM</u> : <b>{(artisan.prenom || "").toUpperCase()}</b></div>
                <div><u>DATE DE NAISSANCE</u> : <b>{formatDateFR(artisan.dateNaissance) || "—"}</b></div>
                <div><u>LIEU DE NAISSANCE</u> : <b>{(artisan.lieuNaissance || "—").toUpperCase()}</b></div>
                <div><u>NUMERO CNI</u> : <b>{artisan.cni || "—"}</b></div>
                <div><u>ZONE DE PROSPECTION</u> : <b>{(artisan.zoneProspection || "Bénoué").toUpperCase()}</b></div>
                <div><u>SUBSTANCE EXPLOITEE</u> : <b>{(artisan.substance || "—").toUpperCase()}</b></div>
              </div>
              <div style={{ width: 70, flexShrink: 0, textAlign: "center", fontSize: 6 }}>
                <div style={{ color: "#1C2B39" }}>N°CARTE :</div>
                <div style={{ fontWeight: 700, fontSize: 6.3, color: "#1C2B39", marginTop: 1, whiteSpace: "nowrap" }}>{artisan.numeroCarte || "—"}</div>
                <div style={{ fontWeight: 700, lineHeight: 1.25, marginTop: 30 }}>Le maire de {commune.label.toUpperCase()}</div>
              </div>
            </div>

            <div style={{ textAlign: "center", fontSize: 6.3 }}>
              <span style={{ position: "relative", left: 12, textDecoration: "underline", fontStyle: "italic" }}>Signature du titulaire</span>
            </div>

            <div style={{ marginTop: 2, marginBottom: 8, fontSize: 6.3, color: "#1C2B39", lineHeight: 1.5 }}>
              <div>Date de délivrance : <b>{formatDateFR(artisan.dateDelivrance) || "—"}</b></div>
              <div>Date d'expiration : <b>{formatDateFR(artisan.dateExpiration) || "—"}</b></div>
            </div>
          </div>
          </div>
          ...
          >
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontWeight: 700, fontSize: 6.3, color: "#1C7A4A", lineHeight: 1.3 }}>(ARTICLE 114, TITRE VI PORTANT CODE MINIER)</div>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "flex-start", flex: 1 }}>
              ...
              <div style={{ fontSize: 6, color: "#5B5346", lineHeight: 1.4 }}>
                Ce QR code permet de vérifier l'authenticité de la carte et les informations du titulaire auprès de la commune de {commune.commune.replace("Commune de ", "")}.
              </div>
            </div>
            ...
            <div style={{ padding: "5px 7px", background: "#F1EBDD", borderRadius: 4, fontSize: 6, fontStyle: "italic", fontWeight: 600, color: "#1C2B39", lineHeight: 1.35 }}>
              NB : «la présente carte ne peut être ni accordée, ni cédée, ni transférée à une tierce autre personne en faute de quoi le contrevenant expose aux sanctions prévues par la loi ».
            </div>
          </div>
          </div>
      </div>
...
function Cartes({ artisans, cartes, setCartes, accessToken, agent }) {
  ...
  return (
    <div>
      ...
      `}</style>
      <SectionHeader title="Carte d'artisan minier" subtitle="Générer la carte recto / verso d'un artisan enregistré" />
      ...
      {mode === "idle" && (
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          >
            <CreditCard size={14} /> Générer une carte
          </button>
          <button
            type="button"
            style={ghostBtn}
      ...
      {mode === "idle" && showRepertoire && (
        <div style={{ ...cardStyle, marginTop: 16, maxWidth: 340 }}>
          <label style={labelStyle}>Choisir un artisan</label>
          ...
          {artisansAvecCarte.length === 0 && (
            <div style={{ fontSize: 12.5, color: "#A99B7F", marginTop: 10 }}>Aucune carte enregistrée pour l'instant.</div>
          )}
        </div>
      )}
      ...
      {mode === "idle" && showRepertoire && artisanRep && communeRep && (
        <div style={{ marginTop: 18 }}>
          ...
          {carteRep?.creePar && (
            <div className="no-print" style={{ textAlign: "center", fontSize: 11.5, color: "#A99B7F", marginTop: 10 }}>
              Générée par <b style={{ color: "#5B5346" }}>{carteRep.creePar}</b>{carteRep.creeLe ? ` le ${formatDateTimeFR(carteRep.creeLe)}` : ""}
            </div>
          )}
      ...
      {mode === "select" && (
        <div style={{ ...cardStyle, marginTop: 16, maxWidth: 340 }}>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            <button type="button" style={primaryBtn} disabled={!genArtisanId} onClick={() => setMode("generated")}>Générer</button>
            <button type="button" style={ghostBtn} onClick={annuler}>Annuler</button>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ marginTop: 18 }}>
          <img src={qrSrc} alt="QR code du site" style={{ borderRadius: 6, border: "1px solid #DCD1B8" }} />
        </div>
        <div style={{ fontSize: 12, color: "#5B5346", marginTop: 12, maxWidth: 420, lineHeight: 1.6 }}>
...
function Agents({ currentAgent, accessToken }) {
  ...
  const removeAgent = async (id) => {
    if (!window.confirm("Supprimer ce compte inspecteur ? (le compte de connexion restera actif, seul le profil sera retiré)")) return;
    try {
  ...
  return (
    <div>
      <SectionHeader
        title="Inspecteurs autorisés"
        subtitle="Les nouveaux comptes se créent depuis l'écran de connexion, via «Créer un compte»"
      />

      {loading && <div style={{ fontSize: 12.5, color: "#A99B7F", marginTop: 10 }}>Chargement...</div>}
        ...
        {agents.map((a) => (
          <div key={a.id} style={{ ...cardStyle, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 14, color: "#1C2B39", fontWeight: 600 }}>
                  {a.prenom ? `${a.prenom} ${a.nom}` : a.nom}{" "}
                  {a.id === currentAgent.id && <span style={{ fontSize: 11, color: "#C9962C", fontWeight: 400 }}>(vous)</span>}
                </div>
                <div style={{ fontSize: 11.5, color: "#5B5346", fontFamily: "'IBM Plex Mono', monospace" }}>
                  {a.login} · {a.matricule || "—"} · {a.role === "admin" ? "Administrateur" : "Inspecteur"}
                </div>
              </div>
            </div>
...
function SectionHeader({ title, subtitle, action }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12, marginBottom: 18 }}>
      <div>
        <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: 24, color: "#1C2B39", margin: 0 }}>{title}</h2>
        {subtitle && <div style={{ fontSize: 13, color: "#5B5346", marginTop: 4 }}>{subtitle}</div>}
      </div>
      {action}
    </div>
  );
...
function EmptyState({ text }) {
  return (
    <div style={{ ...cardStyle, textAlign: "center", color: "#A99B7F", fontSize: 13.5, padding: "28px 18px" }}>
      {text}
    </div>
  );
}
...
export default function App() {
  ...
  return (
    <>
      ...
      {view === "home" && (
        <Home onEnter={() => setView("login")} arrondissement={arrondissement} onOpenMenu={() => setMenuOpen(true)} agent={agent} />
      )}
      {view === "login" && (
